...
### Web Authentication (HTTP-only Cookies)

| Cookie         | Attributes                                              |
|----------------|---------------------------------------------------------|
| `session_id`   | `HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=86400`|
| `csrf_token`   | `Secure; SameSite=Lax; Path=/; Max-Age=86400`          |

- Session stored server-side (Redis or DB), not in the cookie itself.
- Session timeout: 8 hours of inactivity, 24 hours absolute maximum.
- On logout: delete session from server store, clear cookies.

### Android Authentication (JWT)

| Token          | Lifetime  | Storage                           |
|----------------|-----------|-----------------------------------|
| Access token   | 15 min    | In-memory only                    |
| Refresh token  | 30 days   | Android Keystore-backed encrypted storage |
...
- CSRF token set as a non-HttpOnly cookie by the backend on login.
- Frontend reads it and sends it as `X-CSRF-Token` header on all state-changing requests (POST, PUT, PATCH, DELETE).
- Backend validates the header against the cookie value.
- Android is exempt (uses JWT Bearer auth, not cookies).
...
| Layer    | Mechanism                                                  |
|----------|------------------------------------------------------------|
| Client   | React Hook Form + Zod / Flutter form validators            |
...
- CORS: allow only the web app's origin and the Android app's known user agent pattern.
...
| Android unit   | Flutter test                                    |
| Android integration | Flutter integration test                   |
...
| CSRF token missing on state-changing request  | 403 (web only)                         |
...
| Student CRUD                           | Create, view, edit, withdraw            |
| Attendance marking                     | Select session, mark, save              |
| Answer entry                           | Select student, enter answers, submit   |
| Results publishing                     | Calculate → review → publish            |
| Rankings display                       | Per-class, cross-class, tied indicators |
| Report generation                      | Select filters → generate → download    |
...
- [ ] Login works for both admin and teacher on web and Android
...
- [ ] CSRF token validated on web state-changing requests
...
graph LR
    subgraph Vercel
        WEB["React SPA<br/>(static files + CDN)"]
    end
...
|------------------|-------------------------------|--------------------------------------|
| React SPA        | Vercel                        | Auto-deploy from `apps/web/`         |
...
APP_SECRET_KEY=<random-64-char-string>      # Session signing, CSRF
APP_ALLOWED_ORIGINS=https://app.example.com # CORS origins, comma-separated
...
VITE_APP_NAME=JMO Management System
```
...
| CORS origins           | `http://localhost:5173`| Staging URL          | Production URL         |
...
1. Connect GitHub repo → select `apps/web/` as root directory
