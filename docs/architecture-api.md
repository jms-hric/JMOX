...
graph TB
    ...
    subgraph Backend ["Backend (Railway/Fly.io/Render)"]
        ...
        AUTH["Auth Module<br/>Cookie + JWT"]
    end
...
| Decision | Rationale |
|----------|-----------|
| **FastAPI on always-on host** (not Vercel serverless) | OMR processing, connection pooling, background jobs, and WebSocket potential all require a persistent process. Resolves context audit items #16–#18. |
| **React SPA on Vercel** | Static hosting with CDN, automatic deployments, custom domain. The frontend is purely static — no server-side rendering needed. |
...
│   │   │   ├── features/             # Feature modules (students, attendance, etc.)
...
| CSS                | CSS Modules or Vanilla CSS (no Tailwind unless requested) |
...
// CSRF token from cookie set by backend
...
- No accidental coupling to the web framework
...
### 5.1 Web (Cookie-Based)

```
POST /api/v1/auth/login
  ← Set-Cookie: session_id=<signed>; HttpOnly; Secure; SameSite=Lax; Path=/
  ← Set-Cookie: csrf_token=<token>; Secure; SameSite=Lax; Path=/

Subsequent requests:
  → Cookie: session_id=<signed>
  → Header: X-CSRF-Token: <token>  (on POST/PUT/PATCH/DELETE)
```

- Session stored in Redis (or DB) with user_id, role, expiry.
- Session timeout: 8 hours of inactivity, 24 hours absolute.
- CSRF token validated on all state-changing requests.

### 5.2 Android (JWT-Based)
...
- Refresh token: 30 days, rotated on each use, stored in Android Keystore.
- Refresh token revocation on logout or account deactivation.

### 5.3 Unified Authorization
...
```mermaid
sequenceDiagram
    participant T as Teacher (Web/Android)
...
|--------|--------------------|-------------------------------------|----------|
| POST   | `/auth/login`      | Login (returns cookie or JWT)       | Public   |
| POST   | `/auth/logout`     | Logout (invalidate session/token)   | Required |
| POST   | `/auth/refresh`    | Refresh JWT (Android)               | Refresh token |
...
**Login response (web):** Sets session cookie + CSRF cookie, returns:
```json
{
  "user": {
    "id": "uuid",
...
**Login response (Android, `client_type: "android"`):**
```json
{
  "user": { ... },
  "access_token": "eyJ...",
